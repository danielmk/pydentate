...
results_dir = os.path.join(dirname, 'output', 'slow_gamma_input_high_sync_gc_membrane_with_dendrite')
...
temporal_patterns = np.array([inhomogeneous_poisson_process(0.0, duration, dt/1000, np.abs(30), peak_freq, refractory_period=0.001) for x in range(n_inputs)], dtype=object) * 1000
...
output_file.create_array('/', 'gc_membrane', obj=np.array(nw.populations[0].VRecords))
output_file.create_array('/', 'gc_dendrite', obj=np.array(dendrite_vcs))
