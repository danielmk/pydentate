...
data_path = "C:\\Users\\Daniel\\pyDentateData\\pattern_separation_data_local_repeated_input_revised\\seed10000\\scale1000\\net_nofeedbackrev\\"
save_path = data_path
...
for row_idx, x in enumerate(data_list[row_idx_start:row_idx_stop]):
    for col_idx, y in enumerate(data_list[row_idx+row_idx_start:len(data_list)]):
        outputs.append(analysis_main.similarity_measure_leutgeb(x,y,len_bin))
        
outputs = np.array(outputs)
np.savetxt(save_path + "1_leutgeb-measure_matrix_tresolved_len-bin_" + str(len_bin) + ".txt", outputs, delimiter="\t")