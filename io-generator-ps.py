...
for run in runs:
    ...
    nw = net_nofeedbackrev.TunedNetwork(nw_seed, temporal_patterns[0+run:24+run],
    ...
    save_file_name = (str(nw) + "data-paradigm-local-pattern" +
                            "-separation_nw-seed_run_input-frequency_scale_" +
                            str(nw_seed) + '_' +
                            str(run) + '_' + 
    ...
    ap_binary_array = np.zeros((2000,6000), dtype=np.bool)
    ...
    for idx_pp, pp in enumerate(PP_to_GCs[0+run:24+run]):
        for gc in pp:
            for times in temporal_patterns[0+run:24+run][idx_pp]:
    ...
    tuned_fig_file_name = (str(nw) + "_spike-plot_paradigm_local-pattern" +
                           ...
                           str(run).zfill(3) + '_' +
                           str(input_scale).zfill(3) + '_' + str(10000) + '_' +
                           str(nw_seed))
