...
import numpy as np
from pydentate import net_basket_cell_ring_Ih, neuron_tools, oscillations_analysis, spike_to_x
...
results_dir = os.path.join(dirname, 'output', 'on_off_Ih')
...
"""Create the network"""
nw = net_basket_cell_ring_Ih.BasketCellRing(seed+2, temporal_patterns, spatial_patterns, chem_connection_matrix, gap_connection_matrix, rec_weight=rec_weight, n_bcs=n_pvbcs, gap_resistance=6e2, gap_delay=0.0, gap_junctions=gaps_on, pp_bc_weight=pp_bc_weight)  # 7.6e-3
