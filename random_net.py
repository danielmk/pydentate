# -*- coding: utf-8 -*-
"""
This module implements the class RandomNetwork.
The RandomNetwork is a variant of the standard_net.StandardNetwork class
that omits topological connectivity patterns from interneurons to granule cells
...
class RandomNetwork(ouropy.gennetwork.GenNetwork):
    ...
    def __init__(self, seed=None, temporal_patterns=np.array([]), spatial_patterns_gcs=np.array([]),
        ...
        """To achieve random connectivity the target pool is set equal to the
        number of granule cells. Thereby the post_synaptic cell is chosen
        randomly among all granule cells"""
        n_gcs = self.populations[0].get_cell_number()
