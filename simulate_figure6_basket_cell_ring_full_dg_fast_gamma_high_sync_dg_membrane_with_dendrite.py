...
results_dir = os.path.join(dirname, 'output', 'fast_gamma_input_high_sync_gc_membrane_with_dendrite')
...
temporal_patterns = np.array([inhomogeneous_poisson_process(0.0, duration, dt/1000, np.abs(80), peak_freq, refractory_period=0.001) for x in range(n_inputs)], dtype=object) * 1000
...
nw.populations[0].voltage_recording(np.arange(10000))

dendrite_vcs = []

for x in nw.populations[0]:
    for y in x.get_segs_by_name('midd'):
        v_vec = h.Vector()
        v_vec.record(y(0.5)._ref_v)
        dendrite_vcs.append(v_vec)
...
output_file.create_array('/', 'gc_membrane', obj=np.array(nw.populations[0].VRecords).mean(axis=0))
output_file.create_array('/', 'gc_dendrite', obj=np.array(dendrite_vcs).mean(axis=0))
