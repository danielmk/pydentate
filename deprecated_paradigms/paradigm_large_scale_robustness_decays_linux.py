...
for seed in seeds:
    ...
    for run in runs:
        ...
        fig = nw.plot_aps(time=500)
        nw.save_ap_fig(fig, save_dir, 'figure_' + save_file_name)
        seed_input_time_stamps.append(temporal_patterns[0+run:24+run])
        seed_output_time_stamps.append([np.array(x[0]) for x in nw.populations[0].ap_counters])
    ...
    np.savez(save_dir + "time-stamps_" + save_file_name, input_time_stamps = np.array(seed_input_time_stamps), output_time_stamps = np.array(seed_output_time_stamps))
