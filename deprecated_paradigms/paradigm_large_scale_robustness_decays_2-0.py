...
for seed in seeds:
    ...
    for run in runs:
        ...
        nw = net_tunedrevdecaysdelayspeaknorm_linux.TunedNetwork(seed,
        ...
        curr_pp_ts = np.array(tsts(pp_lines, dt_signal=0.1, t_start=0, t_stop=100), dtype =np.bool)
        curr_gc_ts = np.array(tsts(nw.populations[0].get_properties()['ap_time_stamps'], dt_signal=0.1, t_start=0, t_stop=100), dtype=np.bool)
        curr_mc_ts = np.array(tsts(nw.populations[1].get_properties()['ap_time_stamps'], dt_signal=0.1, t_start=0, t_stop=100), dtype=np.bool)
        curr_hc_ts = np.array(tsts(nw.populations[2].get_properties()['ap_time_stamps'], dt_signal=0.1, t_start=0, t_stop=100), dtype=np.bool)
        curr_bc_ts = np.array(tsts(nw.populations[3].get_properties()['ap_time_stamps'], dt_signal=0.1, t_start=0, t_stop=100), dtype=np.bool)
        
        pp_ts.append(curr_pp_ts)
        gc_ts.append(curr_gc_ts)
        mc_ts.append(curr_mc_ts)
        bc_ts.append(curr_hc_ts)
        hc_ts.append(curr_bc_ts)
