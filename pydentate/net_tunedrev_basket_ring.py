...
class TunedNetwork(gennetwork.GenNetwork):
    ...
    def __init__(self, 
        ...
        # Setup cells
        self.mk_population(GranuleCell, 10000)
        self.mk_population(MossyCell, 300)
        self.mk_population(BasketCell, 120)
        self.mk_population(HippCell, 120)
        ...
        gennetwork.tmgsynConnection(self.populations[2], self.populations[1],
                                    140, 'proxd', 3, 3.3, 0, 1, 0, -70, 10,
                                    1.5, 1.5e-3)
        # pdb.set_trace()
        # BC -> BC
        gennetwork.tmgsynConnectionMatrix(self.populations[2], self.populations[2],
                                    rec_matrix, 'proxd', 1.8, 0, 1, 0, -70, 10,
                                    0.8, rec_weight)
        
        if gap_junctions:
            gennetwork.GapJunctionConnectionMatrix(self.populations[2], self.populations[2], gap_matrix, 'proxd', gap_resistance, gap_delay)
