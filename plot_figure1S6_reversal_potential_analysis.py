...
data_dir = os.path.join(dirname, 'output', 'figure_1_S6_reversal')
...
for curr_file in all_files:
    ...
    data_dict['equilibrium'].append(float(curr_file.split("_")[-1].split('.')[0]))
...
min_synapses_idx = df['equilibrium'].argmin()
max_synapses_idx = df['equilibrium'].argmax()
...
ax[0, 0].set_title("Synaptic Equilibrium Potential:\n" + f"{df['equilibrium'].min()}")
...
ax[0, 1].set_title("Synaptic Equilibrium Potential:\n" + f"{df['equilibrium'].max()}")
...
sns.scatterplot(data=df, x='equilibrium', y='mean_pearson_r', ax=ax[0, 2], color='k')
sns.scatterplot(data=df, x='equilibrium', y='k_point_1_over_f', ax=ax[1, 2], color='k')
sns.scatterplot(data=df, x='equilibrium', y='area_over_line', ax=ax[2, 2], color='k')
